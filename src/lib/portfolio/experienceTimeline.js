...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    float radialFade = 1.0 - abs(vUv.y - 0.5) * 0.5;
    ...
    vec3 col = mix(grad * 1.5, grad * 1.0, edge);
    float alpha = clamp((0.88 + head * 0.12) * radialFade, 0.0, 1.0);
...
export class ExperienceTimeline {
  ...
  constructor({
    ...
    // Warm paper-cut accents so the traced line pops against a cool bg.
    this.colorA = "#FFE7B0"; // warm cream (blue-forest paper tone)
    this.colorB = "#FF8A5C"; // coffee-ember warm tip
  ...
  async init(THREE) {
    ...
    const tubeGeo = new THREE.TubeGeometry(curve, 220, 0.085, 12, false);
